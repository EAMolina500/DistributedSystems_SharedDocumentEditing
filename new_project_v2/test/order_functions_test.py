...
from auxiliar_functions import compare, insert_operation
...
class FunctionTest(unittest.TestCase):
  ...
  def test_insert_operation_empty(self):
    ...
    self.assertEqual(insert_operation([], new_operation), [new_operation])
  ...
  def test_insert_in_middle_by_clock(self):
    operations = [
      Operation('insert', 0, 'a', [1, 0, 0], '1A'),
      Operation('insert', 0, 'c', [3, 1, 0], '1C')
    ]
    new_operation = Operation('insert', 0, 'b', [2, 1, 0], '1B')
    expected = [
      operations[0],
      new_operation,
      operations[1]
  ...
  def test_insert_in_middle_by_replica_id(self):
    ...
    new_operation = Operation('insert', 0, 'b', [3, 0, 0], '1B')
    expected = [
      operations[0],
      new_operation,
      operations[1]
